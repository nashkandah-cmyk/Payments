...
def fetch_db():
    ...
    # Firebase stores JSON as stringValue
    for k, v in fields.items():
        if 'stringValue' in v:
            try:
                return json.loads(v['stringValue'])
            except:
                pass
    return {}
...
def generate_ics(db):
    ...
    def uid():
        uid_counter[0] += 1
        return f"em-{uid_counter[0]}@englishmaster"
    ...
    for sid, s in sessions.items():
        ...
        lines += ["BEGIN:VALARM","TRIGGER:-PT30M","ACTION:DISPLAY",f"DESCRIPTION:حصة {ics_escape(name)} بعد 30 دقيقة","END:VALARM","END:VEVENT"]
    ...
    for eid, e in exams.items():
        ...
        lines += [
            ...
            "BEGIN:VALARM","TRIGGER:-PT60M","ACTION:DISPLAY",f"DESCRIPTION:امتحان {ics_escape(name)} بعد ساعة","END:VALARM",
            "END:VEVENT",
        ]
